...
class collect():
    def __init__(self, nav_frame="", axis=9, hz=100, calibration=False, load_calib=False):
        self.alive = True
    ...
    def get_ahrs_euler(self):
        ...
        self.w, self.x, self.y, self.z = self.ahrs.run(
            self.acc, self.gyr, self.mag, self.hz)
    ...
    def update_format(self):
        self.gx_ = self.gyro_raw[1]
        self.gy_ = self.gyro_raw[0]
        self.gz_ = self.gyro_raw[2]

        self.ax_ = self.acc_raw[1]
        self.ay_ = self.acc_raw[0]
        self.az_ = self.acc_raw[2]

        self.mx_ = self.mag_raw[1]
        self.my_ = self.mag_raw[0]
